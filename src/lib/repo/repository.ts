...
import { db, type DBType, type TransactionType } from "@/db/drizzledb";



export class Repository<TEntity, TTable extends MySqlTableWithColumns<any>>  implements IRepository<TEntity> {
    // protected abstract readonly table: MySqlTableWithColumns<any>;
    // protected abstract readonly schema: any;
  
    constructor(
        protected readonly table: TTable,
        protected readonly db: DBType | TransactionType = db
      ) {}
    ...
    async findOne(where?: SQL | undefined): Promise<TEntity | null> {
      const [record] = await this.db.select().from(this.table).where(where).limit(1);
      return (record as TEntity) ?? null;
    }
  
    async findMany(where?: SQL | undefined): Promise<TEntity[]> {
      const records = await this.db.select().from(this.table).where(where);
    ...
    async delete(id: string | number): Promise<void> {
      await this.db.delete(this.table).where(eq(this.table.id as MySqlColumn, id));
    }
